...
public class SixWayJoinEnumerator extends AbstractEnumerator {

    public SixWayJoinEnumerator(CombinationSpace space) {
        super(space);
        this.mode = Constants.QueryType.FIXED.SIX_WAY_JOIN;
    }
    ...
    public void buildSingleQuery(int eventRate, int windowLength, int tupleWidth) {
        ...
        WindowedJoinOperator windowedJoin5 = new WindowedJoinOperator(getOperatorIndex(true), windowedJoinDescription);
        ...
        currentGraph.addEdge(nextEdgeIndex(), currentGraph.getNode(windowedJoin4.getId()), currentGraph.getNode(windowedJoin5.getId()), true);
        currentGraph.addEdge(nextEdgeIndex(), currentGraph.getNode(head5), currentGraph.getNode(windowedJoin5.getId()), true);
